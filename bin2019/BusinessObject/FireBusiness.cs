...
namespace Bin2019.BusinessObject
{
    public partial class FireBusiness : BaseBusiness
    {
		...
		private void SalesEdit(int rowIndex)
		{
			...
			//权限检查
			string s_right = MiscAction.GetRight(Envior.cur_userId,  "01060");
		...
		{
			//权限检查
			if (MiscAction.GetRight(Envior.cur_userId, "01070") == "0")
			{
				MessageBox.Show("权限不足!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
		...
		private void SettleHandle()
		{
			...
			foreach (int i in gridView1.GetSelectedRows())
			{
				...
				invcode = gridView1.GetRowCellValue(i, "INVOICECODE").ToString();
				if (string.IsNullOrEmpty(invcode))
				{
					MessageBox.Show("第" + (i + 1).ToString() + "行项目没有财政发票编码!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
					return;
				}
			...
			frm_settle.swapdata["invtype"] = "F";       //发票类型 -财政
		...
		{
			//权限检查
			if (MiscAction.GetRight(Envior.cur_userId, "01080") == "0")
			{
				MessageBox.Show("权限不足!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
		...
		private void barButtonItem15_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			row = gridView1.LocateByValue("SA002", "06");
			/////如果火化未结算
			if (gridView1.IsValidRowHandle(row) && gridView1.GetRowCellValue(row, "SA008").ToString() == "0")
			{
				for (int i = 0; i < gridView1.RowCount; i++)
				{
					if (gridView1.GetRowCellValue(i, "SA008").ToString() == "1") continue;  //已经结算,忽略
					invcode = gridView1.GetRowCellValue(i, "INVOICECODE").ToString();
					
					if (string.IsNullOrEmpty(invcode))
					{
						MessageBox.Show("第" + (i+1).ToString() + "行数据没有财政发票编码!","提示",MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
						return;
					}
				}
			}
			else if (gridView1.IsValidRowHandle(row) && gridView1.GetRowCellValue(row, "SA008").ToString() == "1")  //火化已经结算
			{
				MessageBox.Show("【火化】业务已经结算完成!","提示",MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
				return;
			} 
			...
			{
				invcode = gridView1.GetRowCellValue(i, "INVOICECODE").ToString(); 
				if (gridView1.GetRowCellValue(i,"SA008").ToString() == "0" &&  (!string.IsNullOrEmpty(gridView1.GetRowCellValue(i, "INVOICECODE").ToString())) )
		...
		private void barButtonItem16_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			for(int i = 0; i < gridView1.RowCount; i++)
			{
				if (gridView1.GetRowCellValue(i, "SA008").ToString() == "1" /*如果结算,忽略*/) continue;
				invcode = gridView1.GetRowCellValue(i, "INVOICECODE").ToString();
				if (string.IsNullOrEmpty(invcode))
				{
					gridView1.SelectRow(i);
			...
			if (gridView1.SelectedRowsCount == 0)
			{
				MessageBox.Show("没有未结算的税务发票销售记录!", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			...
			foreach (int i in gridView1.GetSelectedRows())
			{
				if (gridView1.GetRowCellValue(i,"PRICE") is System.DBNull ||  Convert.ToDecimal(gridView1.GetRowCellValue(i, "PRICE")) == 0)
				{
					MessageBox.Show("要结算的项目尚未输入价格!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
					return;
				}
			...
			frm_settle.swapdata["AC001"] = AC001;
			frm_settle.swapdata["rowList"] = rowList;
			frm_settle.swapdata["invtype"] = "T";       //发票类型 -税务
