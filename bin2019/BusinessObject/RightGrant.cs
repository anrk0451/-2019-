...
namespace Bin2019.BusinessObject
{
	public partial class RightGrant : BaseBusiness
	{
		...
		private void lookup_roles_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
		{
			if (isUpdate)
			{
				if (MessageBox.Show("数据已经改变,是否保存?", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
				{
					if (this.Save() > 0)
						e.Cancel = false;
					else
