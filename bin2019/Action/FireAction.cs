...
namespace Bin2019.Action
{
    class FireAction
    {
		...
		/// <param name="handler"></param>
		/// <returns></returns>
		public static int FireBusinessSettle(string settleId, string ac001,string cuname,string cuid, string[] sa001_arry, string handler,string invtype)
		{
			//结算流水号
			...
			//交款人身份证号
			OracleParameter op_cuid = new OracleParameter("ic_cuid", OracleDbType.Varchar2);
			...
			//结算发票类型
			OracleParameter op_invtype = new OracleParameter("ic_invtype", OracleDbType.Varchar2, 3);
			op_invtype.Direction = ParameterDirection.Input;
			op_invtype.Value = invtype;

			return SqlAssist.ExecuteProcedure("pkg_business.prc_FireBusinessSettle",
				new OracleParameter[] { op_settleId, op_ac001,op_cuname,op_cuid, op_sa001_arry, op_handler,op_invtype });
		...
		/// <param name="handler"></param>
		/// <returns></returns>
		public static int TempSalesSettle(string cuname, string cuid,string settleId, string[] itemId_arry,string[] itemType_arry,decimal[] price_arry,decimal[] nums_arry, 
			string handler,string invtype)
		{
			//交款人姓名
			...
			//交款人身份证号
			OracleParameter op_cuid = new OracleParameter("ic_idcardNo", OracleDbType.Varchar2, 20);
			op_cuid.Direction = ParameterDirection.Input;
			op_cuid.Value = cuid;
			...
			return SqlAssist.ExecuteProcedure("pkg_business.prc_TempSalesSettle",
				new OracleParameter[] { op_cuname,op_cuid, op_settleId,op_itemId_arry,op_itemType_arry,op_price_arry,op_nums_arry,op_handler,op_invtype });
		}
